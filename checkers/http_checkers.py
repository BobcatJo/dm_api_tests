...
def check_registration_email(
    ...
    try:
        ...
        if expected_status_code != requests.codes.OK:
            raise AssertionError(f'Ожидаемый статус код должен быть равен {expected_status_code}')
        if expected_message :
            raise  AssertionError(f'В email должен быть знак @')
    except HTTPError as e:
        assert e.response.status_code == expected_status_code
        assert e.response.json()['title'] == expected_message
...
def check_registration_login(
    ...
    try:
        ...
        if expected_status_code != requests.codes.OK:
            raise AssertionError(f'Ожидаемый статус код должен быть равен {expected_status_code}')
        if expected_message :
            raise  AssertionError(f'В login должно быть минимум 3 символа')
    except HTTPError as e:
        assert e.response.status_code == expected_status_code
        assert e.response.json()['title'] == expected_message

@contextmanager
def check_registration_password(
        expected_status_code: requests.codes = requests.codes.OK,
        expected_message: str = ''):
    try:
        yield
        if expected_status_code != requests.codes.OK:
            raise AssertionError(f'Ожидаемый статус код должен быть равен {expected_status_code}')
        if expected_message :
            raise  AssertionError(f'В password должно быть минимум 6 символов')
    except HTTPError as e:
        assert e.response.status_code == expected_status_code
        assert e.response.json()['title'] == expected_message